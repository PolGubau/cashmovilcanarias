import type { DeviceStatus, OrderStatus, RepairStatus } from "@/lib/supabase/types";
...
export default function StatusBadge({ status, type }: Props) {
  const map: Record<string, [string, string]> =
    type === "device" ? DEVICE : type === "repair" ? REPAIR : ORDER;
  const [style, label] = map[status] ?? ["bg-gray-2 text-gray-7", status];
  return (
