...
export async function getDevices(filters?: { status?: string; brand?: string; search?: string }) {
  const supabase = await createClient();
  let query = supabase.from("v_devices_full").select("*").order("created_at", { ascending: false });

  if (filters?.status) query = query.eq("status", filters.status);
  if (filters?.brand) query = query.ilike("brand", `%${filters.brand}%`);
  if (filters?.search) {
    query = query.or(`imei.ilike.%${filters.search}%,model.ilike.%${filters.search}%,brand.ilike.%${filters.search}%`);
  }
